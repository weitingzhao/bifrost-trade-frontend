...
4. **Anti-generic AI aesthetics (CRITICAL)**
   ...
   - Cliche color schemes: especially **purple/blue gradients on white**, washed-out "default SaaS" palettes.
...
## Project-Specific Notes: Bifrost Trade Frontend + Skote Theme

When working on the **bifrost-trade-frontend** monitoring UI (dashboard, replay & risk pages):

- **Stack & architecture**
  - Keep the React 18 + Vite SPA and FastAPI backend; **do not** migrate to Skote's Node/Express/EJS stack.
...
- **What to borrow from Skote**
  - **Card and panel patterns**: card padding, title row + action buttons, shadows/rounded corners, hover states for dashboards.
  - **Data tables**: compact row height, header tiers, zebra striping, status Badge / Tag usage.
  - **Dashboard layout**: KPI card grids, sidebar + topbar combos, filter/toolbar above tables.
  - **Forms and filter bars**: compact inputs, dropdowns, multi-select/Segmented controls, button hierarchy.
  - **Color emphasis**: success/warning/danger color usage (text, icons, backgrounds), and how to use bold/contrast to highlight key numbers.

- **What NOT to borrow directly**
  - Do not copy Skote's routing, server-side templates, or JS runtime into this project.
  - Do not copy large HTML blocks with inline scripts or unknown dependencies.
  - Do not import Skote's full Vendor CSS/JS bundle; extract only necessary parts and rewrite using existing CSS/components.

- **Target aesthetic for this project**
  - The monitoring UI should be an **industrial-style trading dashboard**: dark theme, high information density, but PnL / risk traffic-lights must be instantly scannable.
  - Functionality and readability first, decoration second; Skote is used to raise "polish level" (cards, tables, spacing, hierarchy), not to override the project's own visual direction.

When designing or refactoring Bifrost Trade UI, combine this Skill's general guidance with the project rules in `.claude/skills/monitoring-ui/SKILL.md` (monitoring/IB account layout, traffic lights, self-check), and treat Skote as a "reference atlas" rather than a template project.
