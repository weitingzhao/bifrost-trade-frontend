# Web Interface Guidelines — vendored reference

**Upstream**: `vercel-labs/web-interface-guidelines` (`command.md`)
**License**: MIT — see `LICENSE.upstream` in this directory.
**Vendored**: 2026-09-09.

Vendored deliberately. Upstream ships a 39-line skill that `WebFetch`es these rules
at run time; that adds a network dependency and an uncontrolled upstream to a
frontend review step. The rules below are the fetched content, unmodified.
Project-specific calibration lives in `SKILL.md`, not here — keep this file a
faithful copy so it can be re-synced against upstream.
