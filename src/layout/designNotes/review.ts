...
export const REVIEW_NOTES: Record<string, string> = {
  '/review':
    ...
    + ' Rev .79 view state (2026-09-25): the window, the quadrant and the open trade are kept for the tab’s session; `state` has no counterpart (the control is fixed to All).',
  '/review/fit':
  ...
  '/review/proposals':
    ...
    + 'deriving the light-theme value here. REBUILT 2026-09-23 against Package 2026-09-23.1, which draws the merge rather than only ruling it. Both of the places this side had given Rule proposals are withdrawn at the design’s word and the Owner’s: the child row under the Inbox, which was the second row §5a.8 deleted wearing an indent, and the fourth View, because a View answers how to read and a proposal is a thing to decide. A proposal is a kind now, `rule`, in the Decisions stream — and `policy_suggestion` is labelled `rule` beside it, because the engine’s rule-keeper and a Review proposal write to the same place. The kind Select is replaced by Writes to (Any · Rules · Policy · Book · Pool · Nothing, each with what is pending there), which runs along the axis the tag colour already used. Policy reads 0 on purpose: the design maps `patch` there and this API has no such kind — what the design calls a patch arrives here as a policy suggestion, which it maps to Rules. The rule card is the design’s: Diff on the left under the 09-22 colour rules, Evidence on the right with the cited trades into Single trade, the habit, n and the dollars on its foot, Distill and Settle off. Neither verdict can be taken — there is no rules store, so Approve → edit Rules and Dismiss are drawn disabled with the reason in the writes slot; the design names Approve, and Dismiss has nowhere to record a no either. Thin proposals (n < 3) are not cards; they would be named on a grey strip with Habits →, and on DEV none is thin. The accordion holds one card open — the first pending one unless you pick — and a folded card has no Approve, so accepting anything means having looked at it. The chain panel went to Habits rather than being deleted with the page it sat on: a broken link is not a decision, it is why a habit cannot argue yet. Measured on DEV: 16 to decide (4 rule changes), one card open, the deep link landing on Writes to = Rules with 11. Owner signed off 2026-09-23 on local DEV (:5173) at rev 2026-09-23.1, with the Decision Inbox it now opens.',
  '/review/objectives':
