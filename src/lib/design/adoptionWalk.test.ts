...
// The walk's own ledger — every state change since the first package, in the
// order it happened. Split from `adoption.test.ts` (2026-09-24) when the record
// crossed the 800-line ratchet: that file keeps the tracker's structural rules;
// this one is the history the counts are pinned to.
describe('the design walk, as it stands', () => {
  it('reads the walk as it stands', () => {
    ...
    // method face, the second — the estimators on real OHLC.
    ...
    // been measured on DEV. 13 with History's method face the same day.
    expect(counts.byState.unbuilt).toBe(13)
