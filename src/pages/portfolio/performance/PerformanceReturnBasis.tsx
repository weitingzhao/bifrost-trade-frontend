...
type BasisRow = { op: string; label: string; value: string; recorded: boolean; strong?: boolean; raw?: number | null }
...
export function PerformanceReturnBasis({
  ...
  const rows: BasisRow[] = [
    ...
    { op: '−', label: 'Cash transactions in range, net', value: fmtSignedUsd0(netCash), recorded: netCash != null },
    ...
    <section className={perfUi.panel} id="return-basis" aria-label="Return basis">
      ...
      <div className="flex flex-wrap gap-x-5.5 gap-y-3 px-3 py-2.25">
        <div className="flex min-w-0 flex-[1_1_20rem] flex-col gap-0.75">
          <span className={cn(perfUi.cap, 'text-foreground/85')}>Balance change is not return</span>
          {rows.map(r => (
            <span key={r.label} className="flex items-baseline gap-2 border-b border-border/40 py-0.5">
              ...
              <span className={cn('text-dense-body', r.strong ? 'text-foreground' : 'text-foreground/70')}>{r.label}</span>
              <span
                className={cn(
                  perfUi.mono,
                  'ml-auto text-dense-body',
                  r.recorded ? 'font-semibold text-foreground/85' : 'text-muted-foreground',
                )}
              >
                {r.value}
              </span>
        ...
        <div className="flex min-w-0 flex-[1_1_18rem] flex-col gap-1.25">
          <span className={cn(perfUi.cap, 'text-foreground/85')}>Which return</span>
          {methods.map(m => (
            <span
              key={m.name}
              className={cn(
                'flex flex-col gap-0.5 rounded-sm border px-2 py-1.5',
                m.headline ? 'border-primary/40' : 'border-border',
              )}
            >
              <span className="flex flex-wrap items-baseline gap-2">
                <span className={cn('text-dense-body font-semibold', m.headline ? 'text-primary' : 'text-foreground')}>
                  {m.name}
                </span>
              ...
              <span className="text-dense-meta text-foreground/70 text-pretty">{m.when}</span>
            </span>
          ))}
          <span className="flex flex-col gap-0.5 rounded-sm border border-dashed border-border px-2 py-1.5">
            <span className="flex flex-wrap items-baseline gap-2">
              <span className="text-dense-body font-semibold text-foreground">Used today · return on capital base</span>
            ...
            <span className="text-dense-meta text-foreground/70 text-pretty">
