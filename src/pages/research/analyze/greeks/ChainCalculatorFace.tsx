/**
 * The chain calculator: one symbol, one trade date, greeks per contract with
 * the Black-Scholes derivation behind each row.
 *
 * It was `/research/greeks`'s page until the design gave that route a
 * different subject — every option leg in the book, which is `BookLegsPanel`.
 * The two are not the same question: this one explores a whole chain on a
 * past date and shows its arithmetic; the design's asks what the book holds
 * right now. Nothing here was changed, only where it sits.
 *
 * Owner call outstanding: the design has no page for this, which is not the
 * same as saying it should go (Owner ruling 2026-09-18). It keeps its place
 * as the second face until the Owner says where it belongs.
 */
...
import { GreeksCalcTooltip } from './GreeksCalcTooltip'
...
export function ChainCalculatorFace() {
