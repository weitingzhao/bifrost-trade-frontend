...
export function MassiveOptionStatusStrip({
  massiveStatus,
}: {
  ...
  return (
    <Card variant="elevated" className="py-3">
      <CardContent className="space-y-2 px-4 py-0">
        <div className="grid gap-4 sm:grid-cols-3 max-w-2xl" aria-label="Connection status">
          ...
          <div className="space-y-0.5">
            <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Option trades</div>
            <div className="text-sm font-medium">{massiveStatus?.trades_enabled ? 'On' : 'Off'}</div>
          </div>
        </div>
