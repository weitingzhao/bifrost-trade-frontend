...
export function buildStructureLegs(input: {
  ...
  wingMid?: number | null
  stepHint?: number | null
}): { legs: RiskPosition[]; coveredShares: number; wing: number | null } {
  ...
  if (kind === 'single') {
    return { legs: [primary], coveredShares: 0, wing: null }
  }
  ...
  if (kind === 'covered') {
    ...
    return { legs: [opt], coveredShares: shares, wing: null }
  }

  // Vertical: buy one, sell the wing.
  const wing = wingStrike(row, spot, input.stepHint)
  const wingPrem =
    input.wingMid != null && Number.isFinite(input.wingMid) && input.wingMid > 0
      ? input.wingMid
      : Math.max(0.05, mid * 0.45)
  ...
  return { legs: [primary, wingLeg], coveredShares: 0, wing }
}
